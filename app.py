...
from get_patch import TempFromTH, HumidityFromTH, Hd, TempA, HumidityA, HdA, OpenFort, Watering, totalHum

parameters = open("parameters.txt", "r", encoding="utf-8")
max_average_temperature, max_average_humidity, max_humidity_in_notch = parameters.readline().split()
parameters.close()
...
def load_interface():
    ...
    quit_button.grid(column=3, columnspan=3, row=6)
    ...
    soil_hum_sensors = tk.Button(text="Датчики влажности почвы", height=7, width=30, font=button_font, bg="white", command=open_soil_humidity_sensors)
    ...
    fork_opener = tk.Button(text="Открыть форточки", height=7, width=30, font=button_font, bg="white", command=open_forks)
    fork_opener.grid(column=0, columnspan=2, row=3)
    fork_closer = tk.Button(text="Закрыть форточки", height=7, width=30, font=button_font, bg="white", command=close_forks)
    fork_closer.grid(column=0, columnspan=2, row=4)
    watering_opener = tk.Button(text="Открыть полив бороздок", height=7, width=30, font=button_font, bg="white", command=open_watering_interface)
    watering_opener.grid(column=2, columnspan=2, row=3)
    watering_closer = tk.Button(text="Закрыть полив бороздок", height=7, width=30, font=button_font, bg="white")
    watering_closer.grid(column=2, columnspan=2, row=4)
    humidifier_opener = tk.Button(text="Открыть систему общего увлажнения", height=7, width=50,
                                  font=button_font, bg="white", command=open_humidifiing_system)
    humidifier_opener.grid(column=4, columnspan=2, row=3)
    humidifier_closer = tk.Button(text="Закрыть систему общего увлажнения", height=7, width=50,
...
def open_temperature_humidity_sensors():    
    ...
    sensors_frame.grid(row=0, column=0, rowspan=5, columnspan=5)
    sensors_label = tk.Label(sensors_root, text="Датчики температуры и влажности", fg="white", bg="#008000", font=label_font)
    sensors_label.grid(row=0, column=0, columnspan=5)

    for i in range(1, 5):
        label_with_number = tk.Label(sensors_root, text=str(i), bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
    ...
    for i in range(2, 4):
        for j in range(1, 5):
            ...
            label_with_data = tk.Label(sensors_root, text=text, bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
            label_with_data.grid(row=i, column=j, sticky=tk.E+tk.W+tk.N+tk.S)

    number_label = tk.Label(sensors_root, text="Номер устройства", bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
    number_label.grid(row=1, column=0, sticky=tk.N+tk.S+tk.E+tk.W)
    temperature_label = tk.Label(sensors_root, text="Температура", bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
    temperature_label.grid(row=2, column=0, sticky=tk.N+tk.S+tk.E+tk.W)
    humidity_label = tk.Label(sensors_root, text="Влажность", bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
    humidity_label.grid(row=3, column=0, sticky=tk.N+tk.S+tk.E+tk.W)
    
    sensors_quit_button = tk.Button(sensors_root, text="Назад", command=sensors_root.destroy, bg="white", font=button_font, height=3, width=7)
    sensors_quit_button.grid(row=4, column=2)
...
def open_soil_humidity_sensors():
    ...
    for i in range(1, 7):
        label_with_number = tk.Label(humidity_root, text=str(i), bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
        label_with_number.grid(row=1, column=i, sticky=tk.E+tk.W+tk.N+tk.S)
        text = Hd(i)
        label_with_data = tk.Label(humidity_root, text=text, bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
        label_with_data.grid(row=2, column=i, sticky=tk.E+tk.W+tk.N+tk.S)

    number_label = tk.Label(humidity_root, text="Номер устройства", bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
    number_label.grid(row=1, column=0, sticky=tk.N+tk.S+tk.E+tk.W)
    soil_humidity_label = tk.Label(humidity_root, text="Влажность почвы", bg="#008000", fg="white", font=label_font, highlightthickness=2, highlightbackground="white")
    soil_humidity_label.grid(row=2, column=0, sticky=tk.N+tk.S+tk.E+tk.W)
    
    sensors_quit_button = tk.Button(humidity_root, text="Назад", command=humidity_root.destroy, bg="white", font=button_font, height=3, width=7)
    sensors_quit_button.grid(row=3, column=3)
...
def open_average_humidity_and_temperature():
    ...
    average_parameters_label = tk.Label(average_parameters_root, text="Датчики влажности почвы", fg="white", bg="#008000", font=label_font)
    average_parameters_label.grid(row=0, column=0)

    average_temperature_text = "Средняя температура: " +  str(TempA())
    average_temperature_label = tk.Label(average_parameters_root, text=average_temperature_text, fg="white", bg="#008000", font=label_font)
    average_temperature_label.grid(row=1, column=0)

    average_humidity_text = "Средняя влажность: " +  str(HumidityA())
    average_humidity_label = tk.Label(average_parameters_root, text=average_humidity_text, fg="white", bg="#008000", font=label_font)
    average_humidity_label.grid(row=2, column=0)

    quit_button = tk.Button(average_parameters_root, text="Назад", bg="white", font=button_font, height=3, width=7, command=average_parameters_root.destroy)
    quit_button.grid(row=3, column=0)


def open_forks():
    global max_average_temperature
    label = root.winfo_children()[16]
    current_average_temperature = TempA()
    if current_average_temperature > float(max_average_temperature):
        OpenFort(1)
...
def show_info_about_temperature():
    ...
    info_frame = tk.Frame(info_root, height=100, width=600, bg="#008000")
...
def open_humidifiing_system():
    ...
    if current_average_humidity < float(max_average_humidity):
        totalHum(1)
...
def open_watering_interface():
    ...
    for i in range(6):
        ...
        button_open = tk.Button(watering_root, text = "Открыть", bg="white", height=3, width=20, font=button_font, command=open_commands[i])
        button_open.grid(row=2, column=i)
        button_close = tk.Button(watering_root, text = "Закрыть", bg="white", height=3, width=20, font=button_font, command=close_commands[i])
        button_close.grid(row=3, column=i)
        current_state = tk.Label(watering_root, text = "Текущее состояние: закрыта", bg="#008000", fg="white", font=small_label_font)
        current_state.grid(row=4, column=i, sticky=tk.N)
        
    quit_button = tk.Button(watering_root, text="Назад", bg="white", font=button_font, height=3, width=7, command=watering_root.destroy)
...
def open_1():
    ...
    if current_humidity_in_notch < float(max_humidity_in_notch):
        Watering(1, 1)
        global watering_root
